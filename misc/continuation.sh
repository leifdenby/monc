	...
	if [ $RUN_MONC_CONFIG -ge 1 ] || [ $RUN_MONC_CP -eq 1 ]; then
		...
		if [ $RUN_MONC_CONFIG -eq 1 ]; then
		    ...
		    eval 'aprun -B $MONC_EXEC --config=$TESTCASE &> $outputfn'
                    ...
                    echo "Reconfigure MONC with configuration file $TESTCASE and its linked xml file,"
                    echo "  starting from checkpoint file $checkpoint_filename"
                    eval 'aprun -B $MONC_EXEC --reconfig=$TESTCASE --checkpoint=$checkpoint_filename --retain_model_time=.true. &> $outputfn'
		else
