    ...
    if (.not. current_state%halo_column) then
       ...
       do k=1, current_state%local_grid%size(Z_INDEX)
          ...
          if (allocated(uprime_tot)) then
             uprime(k) =  &
                  ((current_state%u%data(k,jcol,icol) &
                 - (vertical_grid%olubar(k) - current_state%ugal)))
          ...
          if (allocated(vprime_tot)) then
             vprime(k) =  &
                  ((current_state%v%data(k,jcol,icol) &
                  - (vertical_grid%olvbar(k) - current_state%vgal)))
...
!      NOTE: all "prime_w" values are the prognostic interpolated on to the w levels
       if (trim(advect_flow) .eq. "pw") then 
          do k=1, current_state%local_grid%size(Z_INDEX)-1
             uprime_w_local =  &
                  ...
                  current_state%ugal
             if (allocated(vertical_grid%olubar)) &
                  uprime_w_local = uprime_w_local - &
                  0.5  * ( vertical_grid%olubar(k) + &
                  vertical_grid%olubar(k+1) )
       ...
       if (trim(advect_flow) .eq. "pw") then 
          do k=1, current_state%local_grid%size(Z_INDEX)-1
             vprime_w_local = &
                  ...
                  current_state%vgal
             if (allocated(vertical_grid%olvbar)) &
                  vprime_w_local = vprime_w_local - &
                  0.5  * ( vertical_grid%olvbar(k) + &
                  vertical_grid%olvbar(k+1) )             
       ...
       if (allocated(vertical_grid%olvbar) .and.               &
            allocated(vertical_grid%olubar)) then
       ...
       if (allocated(wke_tot)) then
          do k=1, current_state%local_grid%size(Z_INDEX)
             ...
             wke_tot(k) = wke_tot(k) + 0.25_DEFAULT_PRECISION * (                   &
                  ...
                  * current_state%w%data(k,jcol,icol)      &
                  * current_state%w%data(kp1,jcol,icol)    &
                  * vertical_grid%rhon(kp1)                &
                  +(current_state%w%data(k,jcol,icol)      &
                  + current_state%w%data(km1,jcol,icol))   &
                  * current_state%w%data(k,jcol,icol)      &
                  * current_state%w%data(km1,jcol,icol)    &
                  * vertical_grid%rhon(k) )                &
                  )
          enddo
       endif
...
!   ========================================================================
  ...
  subroutine field_value_retrieval_callback(current_state, name, field_value)
    ...
    if (name .eq. "prefn_local") then
       ...
       do k = 1, current_state%local_grid%size(Z_INDEX)
          field_value%real_1d_array(k)=vertical_grid%prefn(k)
       enddo
    else if (name .eq. "rho_local") then
       allocate(field_value%real_1d_array(current_state%local_grid%size(Z_INDEX)))
       do k = 1, current_state%local_grid%size(Z_INDEX)
          field_value%real_1d_array(k)=vertical_grid%rho(k)
       enddo
    else if (name .eq. "rhon_local") then
       allocate(field_value%real_1d_array(current_state%local_grid%size(Z_INDEX)))
       do k = 1, current_state%local_grid%size(Z_INDEX)
          field_value%real_1d_array(k)=vertical_grid%rhon(k)
       enddo 
    else if (name .eq. "thref_local") then
       allocate(field_value%real_1d_array(current_state%local_grid%size(Z_INDEX)))
       do k = 1, current_state%local_grid%size(Z_INDEX)
          field_value%real_1d_array(k)=vertical_grid%thref(k)
       enddo   
    else if (name .eq. "thinit_local") then
       allocate(field_value%real_1d_array(current_state%local_grid%size(Z_INDEX)))
       do k = 1, current_state%local_grid%size(Z_INDEX)
          field_value%real_1d_array(k)=vertical_grid%theta_init(k)
       enddo    
...
!   2nd stream
       ...
       allocate(field_value%real_1d_array(current_state%local_grid%size(Z_INDEX)))
       do k = 1, current_state%local_grid%size(Z_INDEX)
          field_value%real_1d_array(k)=vertical_grid%u_init(k)
       enddo    
    else if (name .eq. "i_vinit_local") then
       allocate(field_value%real_1d_array(current_state%local_grid%size(Z_INDEX)))
       do k = 1, current_state%local_grid%size(Z_INDEX)
          field_value%real_1d_array(k)=vertical_grid%v_init(k)
       enddo    
