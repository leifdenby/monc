...
module iobridge_mod
  ...
  integer :: sampling_interval_time_3d, sampling_interval_time

  logical :: finished_time_point_send
...
contains 
  ...
  subroutine init_callback(current_state)
    ...
    if (current_state%time_basis) then 
      ...
      if (.not. current_state%normal_step) then
        ...
        if (next_sample_time .eq. current_state%next_sample_time) then
          current_state%sample_timestep = current_state%timestep + sample_nts
        end if
      end if 
    end if ! time_basis=time
  ...
  subroutine timestep_callback(current_state)
    ...
    integer :: i, last_definition_to_send
    

    finished_time_point_send = .false.

    if (.not. io_server_enabled) return

    if (current_state%time_basis) then

      last_definition_to_send = 0
      do i=1, size(data_definitions)
        if (data_definitions(i)%frequency .gt. 0) then
          if ( current_state%timestep == current_state%sample_timestep_3d   &
                    .and.  data_definitions(i)%frequency == sampling_interval_time_3d ) then
            last_definition_to_send = i
          else if ( current_state%timestep == current_state%sample_timestep &
                    .and.  data_definitions(i)%frequency == sampling_interval_time    ) then
            last_definition_to_send = i
          end if
        endif
      end do
      ...
      do i=1, size(data_definitions)
        if (data_definitions(i)%frequency .gt. 0) then
          if ( i .eq. last_definition_to_send) finished_time_point_send = .true.
          if ( current_state%timestep == current_state%sample_timestep_3d   &
                    .and.  data_definitions(i)%frequency == sampling_interval_time_3d ) then
      ...
      if ( current_state%timestep == current_state%sample_timestep_3d ) then
        current_state%next_3d_sample_time = current_state%next_3d_sample_time + sampling_interval_time_3d
      end if
      if ( current_state%timestep == current_state%sample_timestep ) then
        current_state%next_sample_time = current_state%next_sample_time + sampling_interval_time
      end if
  ...
  subroutine finalisation_callback(current_state)
    ...
    integer :: ierr, i, last_definition_to_send
  ...
  integer function pack_scalar_into_send_buffer(current_state, data_definition, field, current_buffer_point)
    ...
    else if (field%name .eq. "finished_time_point_send") then
      pack_scalar_into_send_buffer=pack_scalar_field(data_definition%send_buffer, current_buffer_point, &
           logical_value=finished_time_point_send)
