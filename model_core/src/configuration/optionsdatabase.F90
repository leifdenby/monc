...
module optionsdatabase_mod
  ...
  use logging_mod, only: LOG_ERROR, log_log, log_master_log, LOG_INFO
  implicit none
  ...
  subroutine options_remove_key(options_database, key)
    ...
    ! Per behaviour of options_get_array_size, the option is only treated as an array 
    ! (i.e., when trim(key)//"a_size" exists), which is when array_size .ge. 2.
    ! array_size is 0 when the key doesn't exist with size 1 or as an array.
    ! Further, if options_has_key returns .false., there is no key to remove, so this 
