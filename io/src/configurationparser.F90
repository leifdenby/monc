...
contains
  ...
  subroutine handle_new_data_definition(attribute_names, attribute_values)
    ...
    building_config%data_definitions(current_building_definition)%fields(4)%name="finished_time_point_send"
    building_config%data_definitions(current_building_definition)%fields(4)%namespace=namespace
    building_config%data_definitions(current_building_definition)%fields(4)%field_type=SCALAR_FIELD_TYPE
    building_config%data_definitions(current_building_definition)%fields(4)%dimensions=0
    building_config%data_definitions(current_building_definition)%fields(4)%data_type=BOOLEAN_DATA_TYPE
    building_config%data_definitions(current_building_definition)%fields(4)%optional=.false.
    building_config%data_definitions(current_building_definition)%fields(4)%collective=.false.
    building_config%data_definitions(current_building_definition)%fields(4)%units=""

    current_building_field=5
