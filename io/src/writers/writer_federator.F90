...
contains
  ...
  subroutine initialise_writer_federator(io_configuration, diagnostic_generation_frequency, continuation_run, &
    ...
    do i=1, io_configuration%number_of_writers
      ...
      if (writer_entries(i)%write_time_frequency .gt. 0 ) then
        writer_entries(i)%previous_write_time = reconfig_initial_time &
                                                - mod(reconfig_initial_time,writer_entries(i)%write_time_frequency)
      else 
  ...
  subroutine check_writer_for_trigger(io_configuration, source, data_id, data_dump)
    ...
    integer :: i, timestep
    real(kind=DEFAULT_PRECISION) :: time
    logical :: terminated, finished_time_point_send
    ...
    if (is_field_present(io_configuration, source, data_id, "timestep") .and. &
      ...
      if (is_field_present(io_configuration, source, data_id, "finished_time_point_send")) then
        finished_time_point_send=get_scalar_logical_from_monc(io_configuration, source, data_id, data_dump, &
                                                              "finished_time_point_send")
      else
        finished_time_point_send=.false.
      end if

      if (finished_time_point_send) trigger_count=trigger_count+1
  ...
  subroutine check_writer_trigger(io_configuration, writer_entry_index, timestep, time, terminated, true_finished_time_point_send)
    type(io_configuration_type), intent(inout) :: io_configuration
    integer, intent(in) :: writer_entry_index, timestep
    real, intent(in) :: time
    logical, intent(in) :: terminated, true_finished_time_point_send
    ...
    if (writer_entries(writer_entry_index)%write_on_model_time .and. true_finished_time_point_send) then
      ...
      if (writer_entries(writer_entry_index)%write_timestep_frequency .gt. 0 .and. true_finished_time_point_send) then
